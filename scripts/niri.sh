...
# ──────────────────────────────────────────────
# 5a2. Wine + dependências 32-bit (games Windows)
# ──────────────────────────────────────────────
step "🍷 Instalando Wine e dependências (64/32-bit)..."

info "Wine, mono, gecko, winetricks e libs 32-bit para games..."
echo ""

# multilib precisa estar habilitado para os pacotes lib32-*
if ! grep -q "^\[multilib\]" /etc/pacman.conf 2>/dev/null; then
  warn "multilib não habilitado — habilitando..."
...
sudo pacman -S --needed --noconfirm \
  ...
  libgphoto2 sane unixodbc dosbox

ok "Wine e dependências 32-bit instalados"
quote
...
# ──────────────────────────────────────────────
# Forçar dark mode padrão via autoconfig
