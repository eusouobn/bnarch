...
# ──────────────────────────────────────────────
REPO_RAW="https://raw.githubusercontent.com/eusouobn/bnarch/main/scripts"
...
case "$DE" in
  ...
  Niri)
    ...
    # Copiar niri.sh do repo (baixa automaticamente se necessário)
    if fetch_script niri.sh; then
  ...
  Labwc)
    ...
    # Copiar labwc.sh do repo (baixa automaticamente se necessário)
    if fetch_script labwc.sh; then
